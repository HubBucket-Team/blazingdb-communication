...
public:
    /**
     * The ContextToken class cannot be used as a key due to it is an abstract class.
     * Instead, it is used the variable member type of that class.
     * It is required to change TokenValue to a concrete ContextToken class.
     */
...
public:
     ...
     * The new message queue will be related to the TokenValue.
     ...
     */
    virtual void registerContext(const TokenValue& context_token) = 0;

    /**
     * It is used to destroy a message queue related to the TokenValue.
     * It uses a 'unique lock' with a shared_mutex in order to ensure unique access to the whole structure.
     *
     * @param context_token  identifier for the message queue.
     */
    virtual void deregisterContext(const TokenValue& context_token) = 0;
...
public:
     ...
     * until the server receives a new message with the same TokenValue.
     ...
     */
    virtual std::shared_ptr<Message> getMessage(const TokenValue& context_token) = 0;

    /**
     * It stores the message in the message queue and it uses the TokenValue to select the queue.
     ...
     */
    virtual void putMessage(const TokenValue& context_token, std::shared_ptr<Message>& message) = 0;
